QGPagingToolbarSisExtPst = Ext.extend(Ext.Toolbar, {
    /** 
    ...
    movePrevious : function(criterio){
		...
		if(this.idGrid)
   		...
   		var callBackObtener = function(correcto,datosResultado){
			if(correcto){
				...
				Ext.getCmp('gridSisExt').getStore().proxy =  new Ext.ux.data.PagingMemoryProxy(datosResultado.datos);							    												
	 			Ext.getCmp('gridSisExt').getStore().load({params: {start: 0, limit: 100}});	    				 	
		...
		llamadaAjax('SistemasExternos.do','anterior','busquedaJSON',datosObtener,callBackObtener,true,true);
    },
    ...
    moveNext : function(criterio){    	
		...
		if(this.idGrid)
   		...
   		var callBackObtener = function(correcto,datosResultado){
			if(correcto){
				...
				Ext.getCmp('gridSisExt').getStore().proxy =  new Ext.ux.data.PagingMemoryProxy(datosResultado.datos);							    												
	 			Ext.getCmp('gridSisExt').getStore().load({params: {start: 0, limit: 50}});	    				 	
	 			Ext.getCmp('gridSisExt').getView().refresh();
		...
		llamadaAjax('SistemasExternos.do','siguiente','busquedaJSON',datosObtener,callBackObtener,true,true);
    ...
    onDestroy : function(){
        this.bindStore(null);
        QGPagingToolbarSisExtPst.superclass.onDestroy.call(this);
    }
    
}); 
   

Ext.reg('cgpaging', QGPagingToolbarSisExtPst);

if(QGPagingToolbarSisExtPst){
  Ext.apply(QGPagingToolbarSisExtPst.prototype, {
