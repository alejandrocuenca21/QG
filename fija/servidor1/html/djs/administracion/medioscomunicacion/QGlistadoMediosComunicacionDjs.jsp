...
var CGLOBAL = function (){
   ...
   var callBackObtener = function(correcto,datosResultado){
		...
		if (correcto){
	    	Ext.getCmp('gridMediosComunicacion').getStore().loadData(datosResultado.datos);
	    }
	    else{				
    ...
    var obtenerDatos = function (){
		...
		Ext.getCmp('gridMediosComunicacion').getSelectionModel().clearSelections(true);

		llamadaAjax('MediosComunicacion.do','buscar',null,null,callBackObtener,true,true);	
    }
    ...
    var modificarDatos = function (){
		...
		llamadaAjax('MediosComunicacion.do','modificar','medioComunicacionJSON',medios,callBackGenerarMod,false);
    }
    ...
    var altaDatos = function (){
		...
		llamadaAjax('MediosComunicacion.do','alta','medioComunicacionJSON',medios,callBackGenerarAlta,false);
    }
   ...
   // Controlamos los eventos sobre los botones
	...
	var imprimirMediosComunicacion = function() {
		...
		document.getElementById('ifExportar').src = contexto + 'MediosComunicacion.do?exportarPDF=';
	}
		...
		if (alta){
			...
			if (!existe){
				...
				Ext.Msg.show({
				   ...
				   msg: '<span>Ya existe un medio de comunicación con ese código.</span>',
				   buttons: Ext.Msg.OK,
				   icon: Ext.MessageBox.ERROR
				});
			}
