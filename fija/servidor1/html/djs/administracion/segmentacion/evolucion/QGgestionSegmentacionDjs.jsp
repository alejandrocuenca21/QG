...
var CGLOBALGESTSEG = function() {
	...
	<sec:authorize ifAnyGranted="ROLE_AD,ROLE_SA, ROLE_AS">	
		...
		if (ultimaBusqueda != null && ultimaBusquedaTipo != null
			...
			if (indexUnidad == 0){
				limpiarCombos();
				indexCodigoSegmentoOrigen = 0;
				indexCodigoSubSegmentoOrigen = 0;
			}
			else{	
				if (indexCodigoSegmentoOrigen == 0){
	...
	var controlEventos = function() {
		...
		Ext.get('selUnidadAS').on('change', function(){	
			limpiarCombos();		 
			var unidad = Ext.get('selUnidadAS').dom.options[Ext.get('selUnidadAS').dom.selectedIndex].value;
			if (unidad != ""){
				CGLOBAL.cargarCodigoSegmento(unidad);
			}
		});
