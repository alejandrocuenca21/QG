   ...
   // Controlamos los eventos sobre los botones
	var controlEventos = function() {
		...
		Ext.get('btnEliminar').on('click', function() {
			if (Ext.getCmp('gridTiposUbicacion').getSelectionModel().hasSelection()) {
				...
				if (dFechaInicio >= dFechaFin){
					...
					if (alta){
	    				...
	    				if (!existe){
							...
							Ext.Msg.show({
							   title:'Error',
							   msg: '<span class="bold">Ya existe un tipo de ubicación con ese código.</span>',
							   buttons: Ext.Msg.OK,
							   icon: Ext.MessageBox.ERROR
							});
						}
