 ...
 *
 */
public class QGBuscadorClientesDaoNA extends QGBaseDao implements
		QGBuscadorClientesDao {
	private Logger logger = Logger.getLogger(QGBuscadorClientesDaoNA.class);
	/**
	 * Se llamará al servicio de negocio QGF0012
	 */

	public QGCGlobalDto obtenerDatosCliente(String codigoCliente) {
		logger.info("******---obtenerDatosCliente de QGBuscadorClientesDaoNA---******");		
		...
		logger.info("******---FIN obtenerDatosCliente de QGBuscadorClientesDaoNA---******");		
		return globalDto;
	}	
	...
	public QGCGlobalDto obtenerDireccionesCliente(String codigoCliente) {
		logger.info("******---obtenerDireccionesCliente de QGBuscadorClientesDaoNA---******");		
		QGNpaNA servicio = null;
		QGCGlobalDto globalDto = new QGCGlobalDto();
		QGUsuario usuarioLogado = obtenerUsuarioLogado();
		ArrayList listaDatos = new ArrayList();
	...
	public QGCGlobalDto obtenerListadoClientes(QGBuscadorClientesDto buscadorClientesDto) {
		logger.info("******---obtenerListadoClientes de QGBuscadorClientesDaoNA---******");
	...
	public QGCGlobalDto obtenerTipoDocumento(String codigoLinea, String codigoModalidad, String codigoDocumento) {
		logger.info("******---obtenerTipoDocumento de QGBuscadorClientesDaoNA---******");		
		QGNpaNA servicio = null;
		QGCGlobalDto globalDto = new QGCGlobalDto();
		QGUsuario usuarioLogado = obtenerUsuarioLogado();
		ArrayList listaDatos = new ArrayList();
		...
		try {
			logger.info("******---antes de crear el na QGF0014---******");
			servicio = new QGNpaNA("QGF0014");
			logger.info("******---después de crear el na QGF0014---******");
			
			servicio.setValor("NACabAnagramaLlamante", "QG");
			servicio.setValor("CACV_USUARIO_SESION", usuarioLogado.getUsername());
			
			servicio.setValor("CO-LIN-NGC", codigoLinea);			
			servicio.setValor("IN-MODALIDAD-UNM", codigoModalidad);
			logger.info("******---" + codigoDocumento + "---******");
			servicio.setValor("CO-TIPO-DOCUMENTO", codigoDocumento);
			
			logger.info("antes de ejecutar transacción");
			servicio.ejecutarTransaccion();
			logger.info("después de exe trans");
			QGDescripcionesDto descripciones = null;
			for(int i=1; i <= servicio.getValorAsInt("SALIDA.OCU-NU").intValue(); i++){
				descripciones = new QGDescripcionesDto();
				
				descripciones.setLinea(servicio.getValorAsString("CO-LIN-NGC", i));
				descripciones.setModalidad(servicio.getValorAsString("IN-MODALIDAD-UNM", i));
				descripciones.setCodigo(servicio.getValorAsString("CO-TIPO-DOCUMENTO", i));
				descripciones.setDescripcion(servicio.getValorAsString("DS-TIPO-DOCUMENTO", i));
				
				listaDatos.add(descripciones);
			}
			logger.info("descripciones --> " + listaDatos.size());
		} catch (NAWRException e) {
			logger.info("error dao --> " + e.getMessage(),e);
		...
		globalDto.setlistaDatos(listaDatos);
		globalDto.setlistaMensajes(listaMensajes);
		logger.info("******---FIN obtenerTipoDocumento de QGBuscadorClientesDaoNA---******");		
		return globalDto;
	}
